...
class ChatGPT:
    ...
    async def send_message(self, message, chat_mode,dialog_messages=[]):
    ...
    async def send_message_stream(self, message,chat_mode, dialog_messages=[]):
        if chat_mode not in config.chat_modes.keys():
            raise ValueError(f"Chat mode {chat_mode} is not supported")
...
async def generate_images(prompt, n_images=1):
    r = openai.Image.create(prompt=prompt, n=n_images, size="512x512")
